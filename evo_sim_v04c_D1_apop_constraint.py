...
def coupling_penalty(params):
    ...
    # D1: Apoptosis constraint — prevent egl1_sens AND cell_survive from both exceeding 3x wild-type
...
if __name__ == "__main__":
    print(f"{'='*65}")
    print(f"  C. elegans OS v0.4c — D1: Apoptosis Constraint")
    print(f"  Penalty: egl1_sens>3x AND cell_survive>3x -> +1.0")
    print(f"  Baseline: 50pop, 0.12mut, 100代")
    print(f"{'='*65}")

    pop, traj, tips, elapsed = evolve("D1_apop_constraint", n_gens=100, pop_size=50, mut_rate=0.12)
    csv_path, rpt_path = save_outputs(pop, traj, tips, elapsed, "D1_apop_constraint")
