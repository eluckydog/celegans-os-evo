...
def task_ac_vu(params, n_seeds=3):
    ...
    for seed in range(n_seeds):
        ...
        K_base, dt = 5.0, 0.1
        # D2: Notch self-inhibition feedback — high Notch suppresses its own ligand
        k_self_inhibit = 0.01
        N1_p, N2_p = N1, N2
        for step in range(250):
            raw1 = G2 * params["notch_sens"] * params["dsl_basal"]
            raw2 = G1 * params["notch_sens"] * params["dsl_basal"]
            # D2: self-inhibition term — high Notch reduces ligand production
...
if __name__ == "__main__":
    print(f"{'='*65}")
    print(f"  C. elegans OS v0.4c — D2: Notch Self-Inhibition")
    print(f"  Self-feedback: dN += -k*N*G (k=0.01)")
    print(f"  Baseline: 50pop, 0.12mut, 100代")
    print(f"{'='*65}")

    pop, traj, tips, elapsed = evolve("D2_notch_feedback", n_gens=100, pop_size=50, mut_rate=0.12)
    csv_path, rpt_path = save_outputs(pop, traj, tips, elapsed, "D2_notch_feedback")
