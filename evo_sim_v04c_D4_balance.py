...
def fitness_30(net, n_seeds=3, verbose=False):
    ...
    # D1 baseline: apoptosis constraint
    if net.params["egl1_sens"] > 3.0 and net.params["cell_survive"] > 3.0:
        total -= 1.0
    # D4: parameter balance penalty — prevent over-reliance on few params
    norm_vals = [net.params[k] / WILD_PARAMS[k] for k in WILD_PARAMS]
    std_val = float(np.std(norm_vals))
    if std_val > 2.0:
        total -= 0.3 * (std_val - 2.0)
...
if __name__ == "__main__":
    ...
    print(f"  {len(COUPLING_RULES)}耦合规则 | 6子任务 | 1方向 x 100代")
    print(f"{'='*65}")

    dirs = {
        "D4_balance": {"mut_rate": 0.12, "pop_size": 50},
    }
