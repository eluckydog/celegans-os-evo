...
def coupling_penalty(params):
    ...
    # D3: Apoptosis constraint — prevent egl1_sens AND cell_survive from both exceeding 3x wild-type
    egl1_r = params["egl1_sens"] / WILD_PARAMS["egl1_sens"]
    cs_r = params["cell_survive"] / WILD_PARAMS["cell_survive"]
    if egl1_r > 3.0 and cs_r > 3.0:
        penalty += 1.0
    return penalty
...
def task_ac_vu(params, n_seeds=3):
    ...
    for seed in range(n_seeds):
        ...
        K_base, dt = 5.0, 0.1
        # D3: Notch self-inhibition feedback
        k_self_inhibit = 0.01
        N1_p, N2_p = N1, N2
        for step in range(250):
            raw1 = G2 * params["notch_sens"] * params["dsl_basal"]
            raw2 = G1 * params["notch_sens"] * params["dsl_basal"]
            # D3: self-inhibition term
            N1 += dt * (params["lin12_expr"]*alpha*raw1*(1-N1) - params["notch_decay"]*N1 - k_self_inhibit*N1*G2 + 0.001*(rng.rand()-0.5))
            N2 += dt * (params["lin12_expr"]*alpha*raw2*(1-N2) - params["notch_decay"]*N2 - k_self_inhibit*N2*G1)
...
if __name__ == "__main__":
    print(f"{'='*65}")
    print(f"  C. elegans OS v0.4c — D3: Hybrid (Apop Constraint + Notch Self-Inhibition)")
    print(f"  Penalty + Self-feedback k=0.01")
    print(f"  Baseline: 50pop, 0.12mut, 100代")
    print(f"{'='*65}")

    pop, traj, tips, elapsed = evolve("D3_hybrid", n_gens=100, pop_size=50, mut_rate=0.12)
    csv_path, rpt_path = save_outputs(pop, traj, tips, elapsed, "D3_hybrid")
    print(f"\n  CSV: {csv_path.name} | RPT: {rpt_path.name}")
    print(f"  Done.")
